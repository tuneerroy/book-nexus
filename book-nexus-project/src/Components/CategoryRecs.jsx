    ...
    setGetAuthors(() => async ({ pageSize, page }) => {
      try {
        const response = await fetch(
          `/api/authors/recommendations/category?including=${included.join(
            ","
          )}&excluding=${excluded.join(
            ","
          )}&andMode=${andMode}&pageSize=${pageSize}&page=${page}`
        );
    ...
    <div style={{ paddingTop: 100, margin: "auto" }}>
      <Box sx={{ maxWidth: 600, margin: "auto" }}>
        ...
        <Typography variant="h7" sx={{ textAlign: "left" }}>
          Include or exclude any genres from your personalized recommendations
          by listing them below.
        </Typography>
        <form onSubmit={handleSubmit}>
          {warning && (
            <Alert severity="warning">
              Can't overlap included and excluded genres!
            </Alert>
          )}
          <Box display="flex" flexDirection="column">
            ...
            <Button
              type="submit"
              variant="contained"
              sx={{ backgroundColor: "#0096c7", marginBottom: 3 }}
            >
              Filter
            </Button>
          </Box>
