...
router.get('/', (req, res) => {
  const query = `
    ...
    GROUP BY isbn
      HAVING ${helpers.fColInRange('AVG(rating)', req.query.rating_low, req.query.rating_high)}
...
router.get('/:isbn', (req, res) => {
  ...
  db.query(query, (err, results) => {
    ...
    results[0].categories = results[0].categories && results[0].categories.split(';');
    results[0].authors = results[0].authors && results[0].authors.split(';').map((author) => {
      const [id, name] = author.split('|');
      return {id, name};
    });
